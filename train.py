...
from utils.loss_utils import l1_loss, tv_loss, smooth_regularization_loss
...
def scene_reconstruction(dataset, opt, hyper, pipe, testing_iterations, saving_iterations, 
    ...
    for iteration in range(first_iter, final_iter+1):        
        ...
        depth_loss = l1_loss((depth / depth.max())[mask!=0], (gt_depth / gt_depth.max())[mask[0,:,:]!=0])
        psnr_ = psnr(image_tensor, gt_image_tensor, mask_tensor).mean().double()        
        loss = Ll1 + depth_loss 
        
        # unsupervise smooth
        depth_tvloss = smooth_regularization_loss(depth, mask) + tv_loss(depth * torch.bitwise_not(mask))  * 30
        img_tvloss = tv_loss(image * torch.bitwise_not(mask)) * 30
        smooth_loss = 0.0001 * (img_tvloss + depth_tvloss)
        if iteration > 1000:
            loss += smooth_loss
