...
def readImages(renders_dir, gt_dir, depth_dir, gtdepth_dir, masks_dir):
    ...
    for fname in os.listdir(depth_dir):
        depth = np.array(Image.open(depth_dir / fname).convert("L"))
        depths.append(torch.from_numpy(depth).unsqueeze(0).unsqueeze(1)[:, :, :, :].cuda())
    for fname in os.listdir(gtdepth_dir):
        gt_depth = np.array(Image.open(gtdepth_dir / fname).convert("L"))
        gt_depths.append(torch.from_numpy(gt_depth).unsqueeze(0).unsqueeze(1)[:, :3, :, :].cuda())
    for fname in os.listdir(masks_dir):
        mask = np.array(Image.open(masks_dir / fname).convert("L"))
        ...
        for scene_dir in model_paths:
            ...
            for method in os.listdir(test_dir):
                ...
                for idx in tqdm(range(len(renders)), desc="Metric evaluation progress"):
                    ...
                    rmse_log = (torch.log(gt_depth) - torch.log(depth)) ** 2
                    rmse_log[mask == 0] = float("nan")
                    rmse_log = torch.sqrt(rmse_log).nanmean()

                    abs_rel = torch.abs(gt_depth - depth) / gt_depth
                    abs_rel[mask == 0] = float("nan")
                    abs_rel = abs_rel.nanmean()
                    sq_rel = (gt_depth - depth) ** 2 / gt_depth
                    sq_rel[mask == 0] = float("nan")
                    sq_rel = sq_rel.nanmean()
                ...
                print("Scene: ", scene_dir,  "RMSE LOG: {:>12.7f}".format(torch.tensor(rmse_logs).mean(), ".5"))
                print("Scene: ", scene_dir,  "ABS REL: {:>12.7f}".format(torch.tensor(abs_rels).mean(), ".5"))
                print("Scene: ", scene_dir,  "SQ REL: {:>12.7f}".format(torch.tensor(sq_rels).mean(), ".5"))
                print("")
                ...
                full_dict[scene_dir][method].update({"SSIM": torch.tensor(ssims).mean().item(),
                                                        ...
                                                        "RMSE LOG": torch.tensor(rmse_logs).mean().item(),
                                                        "ABS REL": torch.tensor(abs_rels).mean().item(),
                                                        "SQ REL": torch.tensor(sq_rels).mean().item()
                                                    })
