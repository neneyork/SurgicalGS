...
class DepthEstimator(torch.nn.Module):
    ...
    def forward(self, imagel, imager, baseline, upsample=True):
        ...
        depth = baseline[:, None, None] / -flow[:, 0] / 2
...
def reformat_dataset(data_dir, start_frame, end_frame, img_size=(512, 640)):
    ...
    frames = sorted(os.listdir(os.path.join(data_dir, 'img_left')))
    ...
    for i, frame in enumerate(frames):
        left_img = torch.from_numpy(cv2.cvtColor(cv2.imread(os.path.join(data_dir, 'img_left', frame)), cv2.COLOR_BGR2RGB)).permute(2, 0, 1).float()
        right_img = torch.from_numpy(cv2.cvtColor(cv2.imread(os.path.join(data_dir, 'img_right', frame.replace('l', 'r'))), cv2.COLOR_BGR2RGB)).permute(2, 0, 1).float()
...
if __name__ == "__main__":
    ...
    # python stereomis2endonerf.py -d /root/autodl-tmp/StereoMIS -f 1 110